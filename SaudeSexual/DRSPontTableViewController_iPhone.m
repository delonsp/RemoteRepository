...
- (void)viewDidLoad
{
    [super viewDidLoad];
    if (self.myArray == nil) {
        NSString *path = [[NSBundle mainBundle] pathForResource:@"Nomes Questionários 2" ofType:@"plist"];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == 0) {
        ...
        quest = [self.questArray objectAtIndex:[indexPath row]-1];
        detailViewController = [[DRSBarGraphViewController alloc] initWithNibName:@"DRSBarGraphViewController" bundle:nil];
//        detailViewController = [[DRSPontuacoesViewController_iPhone alloc] initWithNibName:@"DRSPontuacoesViewController_iPhone" bundle:nil];
    }
